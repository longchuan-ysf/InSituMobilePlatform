...
/*! 
* \brief  添加指令数据
* \detial 串口接收的数据，通过此函数放入一个字节指令队列 
*  \param  _data 指令数据
*/
void queue_pushChar(qdata _data)
{
...
/*! 
* \brief  添加指令数据
* \detial 串口接收的数据，通过此函数放入一串指令队列 
*  \param  _data 指令数据指针
*/
void queue_pushString(qdata *_pdata,uint32 len)
{
	uint32 i;
	qdata qdata;
	for(i=0; i<len; i++)
